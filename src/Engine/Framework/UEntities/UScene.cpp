...
void UScene::registerUObject(const std::shared_ptr<UObject>&){
	this->uObjects.push_back(ptr);
}

void UScene::unregisterUObject(const std::shared_ptr<UObject>&){
	this->uObjects.remove(ptr);
}